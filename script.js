...
function initMap() {
  ...
  for(i=0; i<countryArray.length; i++){
    map.data.loadGeoJson('countries/'+countryArray[i]+'.geojson');
    map.data.setStyle({
      fillColor: "#fc0352",
      strokeColor: "#fc0352"
    });
...
function checkInput(){
    ...
    for(i=0; i<countryArray.length; i++){
      if(userstuff==countryArray[i]){
        ...
        map.data.remove('countries/'+userstuff+'.geojson');
...
PROBLEMS I SHOULD BE FIXING
  ...
  - make geo jsons dissapear not reveal <<<< PRIORITY 7
